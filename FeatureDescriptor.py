...
def ReadFileFromFasta(filepath):
    ...
    for seq_record in SeqIO.parse(filepath, "fasta"):
        seqs.append(['>' + seq_record.id.strip(), str(seq_record.seq).strip()[:-1]]) 
    return seqs
...
def FeatureGeneratorTT(fastas):
    ...
    for i in fastas:
        name = i[0]
        if str(name).startswith('>') and str(name).find('lcl') != -1:
            labels.append(0)
        else:
            labels.append(1)
    ...
    FeatureDict_sORFs['ESMER3'] = np.array(ESMER3[1:, 1:], dtype=float)
    FeatureNameDict['ESMER3'] = ESMER3[:1, :][0]

    FeatureDict_sORFs['ESMER4'] = np.array(ESMER4[1:, 1:], dtype=float)
    FeatureNameDict['ESMER4'] = ESMER4[:1, :][0]

    FeatureDict_sORFs['ESMER5'] = np.array(ESMER5[1:, 1:], dtype=float)
    ...
    #sORFs
    FeatureDict_sORFs['kmer'] = scaler.fit_transform(np.array(kmer[1:, 1:], dtype=float))
    FeatureNameDict['kmer'] = kmer[:1, :][0]
    
    FeatureDict_sORFs['ssm'] = scaler.fit_transform(np.array(ssm[1:, 1:], dtype=float))
    FeatureNameDict['ssm'] = ssm[:1, :][0]
    
    FeatureDict_sORFs['ggap1'] = scaler.fit_transform(np.array(ggap1[1:, 1:], dtype=float))
    FeatureNameDict['ggap1'] = ggap1[:1, :][0]
    
    FeatureDict_sORFs['ggap2'] = scaler.fit_transform(np.array(ggap2[1:, 1:], dtype=float))
    FeatureNameDict['ggap2'] = ggap2[:1, :][0]
    ...
    #aa
    FeatureDict_aas['aac'] = scaler.fit_transform(np.array(aac[1:, 1:], dtype=float))
    FeatureNameDict['aac'] = aac[:1, :][0]

    FeatureDict_aas['dpc'] = scaler.fit_transform(np.array(dpc[1:, 1:], dtype=float))
    FeatureNameDict['dpc'] = dpc[:1, :][0]

    FeatureDict_aas['asdc'] = scaler.fit_transform(np.array(asdc[1:, 1:], dtype=float))
    FeatureNameDict['asdc'] = asdc[:1, :][0]

    FeatureDict_aas['c1saap'] = scaler.fit_transform(np.array(c1saap[1:, 1:], dtype=float))
    FeatureNameDict['c1saap'] = c1saap[:1, :][0]

    FeatureDict_aas['c2saap'] = scaler.fit_transform(np.array(c2saap[1:, 1:], dtype=float))
    FeatureNameDict['c2saap'] = c2saap[:1, :][0]

    FeatureDict_aas['c3saap'] = scaler.fit_transform(np.array(c3saap[1:, 1:], dtype=float))
    FeatureNameDict['c3saap'] = c3saap[:1, :][0]

    FeatureDict_aas['gaac'] = scaler.fit_transform(np.array(gaac[1:, 1:], dtype=float))
    FeatureNameDict['gaac'] = gaac[:1, :][0]

    FeatureDict_aas['gdpc'] = scaler.fit_transform(np.array(gdpc[1:, 1:], dtype=float))
    FeatureNameDict['gdpc'] = gdpc[:1, :][0]

    FeatureDict_aas['gtpc'] = scaler.fit_transform(np.array(gtpc[1:, 1:], dtype=float))
    FeatureNameDict['gtpc'] = gtpc[:1, :][0]

    FeatureDict_aas['c1saagp'] = scaler.fit_transform(np.array(c1saagp[1:, 1:], dtype=float))
    FeatureNameDict['c1saagp'] = c1saagp[:1, :][0]

    FeatureDict_aas['c2saagp'] = scaler.fit_transform(np.array(c2saagp[1:, 1:], dtype=float))
    FeatureNameDict['c2saagp'] = c2saagp[:1, :][0]

    FeatureDict_aas['c3saagp'] = scaler.fit_transform(np.array(c3saagp[1:, 1:], dtype=float))
    FeatureNameDict['c3saagp'] = c3saagp[:1, :][0]

    FeatureDict_aas['ctdc'] = scaler.fit_transform(np.array(ctdc[1:, 1:], dtype=float))
    FeatureNameDict['ctdc'] = ctdc[:1, :][0]

    FeatureDict_aas['ctdt'] = scaler.fit_transform(np.array(ctdt[1:, 1:], dtype=float))
    FeatureNameDict['ctdt'] = ctdt[:1, :][0]

    FeatureDict_aas['ctdd'] = scaler.fit_transform(np.array(ctdd[1:, 1:], dtype=float))
    FeatureNameDict['ctdd'] = ctdd[:1, :][0]
    return FeatureDict_sORFs, FeatureDict_aas, np.array(labels).astype(int), FeatureNameDict

def FeatureGenerator(fastas, flag='train-validate'):
    FeatureDict_sORFs, FeatureDict_aas, labels, FeatureNameDict = FeatureGeneratorTT(fastas)
    if flag == 'train-validate':
